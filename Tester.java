 ...
 * 
 * See http://www.junit.org/apidocs/org/junit/Assert.html for the available
...
public class Tester {
	...
	public void TestGeneralLedger() throws Exception{
		...
		/*//start tangent idea
		c3.setType("something else");
		assertTrue("after clearing cache 3rd time", compareTwoObjects(c.getClass(), c3, c));	
*/
...
//		p.setprodType("Conceptual Product");
			...
			for (Map.Entry<String, Class> entry : firstMap.entrySet()) {
				...
				if (fieldType == String.class) {
					...
					throw new DataException(
							"Cannot read "
									+ boClass.getName()
									+ " from the database.  The BusinessObjectDAO does not support this data type: "
									+ fieldType.getName());
				}
	 ...
	 * @throws Exception
	 */
	private Map<String, Class> getBusinessObjectFields(Class boClass)
			throws Exception {
	...
	private Method getGetterMethod(Class boClass, String fieldName,
			Class fieldType) throws DataException {
		...
		throw new DataException("Error in " + boClass.getName()
				+ ".  No method named " + getterName + "("
				+ fieldType.getName() + ") to correspond with field "
